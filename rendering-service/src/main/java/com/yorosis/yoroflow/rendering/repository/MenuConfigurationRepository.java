...
public interface MenuConfigurationRepository extends JpaRepository<Menu, UUID> {
	...
	public List<Menu> getMenuList(Pageable pageable, @Param("activeFlag") String flag,
	...
	public Menu getMenuByApplicationIdentifier(@Param("id") String id, @Param("activeFlag") String flag,
			@Param("tenantId") String tenantId);

	@Query("select c from Menu c where c.activeFlag=:activeFlag and c.tenantId=:tenantId and c.managedFlag=:managedFlag")
	public List<Menu> getMenuListWithManageFlag(@Param("activeFlag") String flag, @Param("tenantId") String tenantId,
			@Param("managedFlag") String managedFlag);

	public Menu findByApplicationIdAndActiveFlagAndTenantId(UUID applicationId, String activeFlag, String tenantId);
