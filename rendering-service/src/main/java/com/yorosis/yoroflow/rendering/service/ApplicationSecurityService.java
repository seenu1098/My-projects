...
import com.yorosis.yoroflow.rendering.repository.ApplicationPermissionsRepository;
...
public class ApplicationSecurityService {
	...
	private ApplicationPermissions constructApplicationPermissionsVOToDTO(PermissionVO appPermissionsVO) {
		...
		return ApplicationPermissions.builder().tenantId(YorosisContext.get().getTenantId())
				.readAllowed(booleanToChar(appPermissionsVO.getReadAllowed()))
				.updateAllowed(booleanToChar(appPermissionsVO.getUpdateAllowed())).createdOn(timestamp)
				.activeFlag(YoroappsConstants.YES).createdBy(YorosisContext.get().getUserName())
				.createAllowed(booleanToChar(appPermissionsVO.getCreateAllowed()))
				.modifiedBy(YorosisContext.get().getUserName()).modifiedOn(timestamp)
				.deleteAllowed(booleanToChar(appPermissionsVO.getDeleteAllowed()))
				.editAllowed(booleanToChar(appPermissionsVO.getEditAllowed()))
				.launchAllowed(booleanToChar(appPermissionsVO.getLaunchAllowed())).build();
	}
	...
	private PermissionVO constructApplicationPermissionsDTOToVO(ApplicationPermissions pagePermissions) {
		return PermissionVO.builder().readAllowed(charToBoolean(pagePermissions.getReadAllowed()))
				...
				.deleteAllowed(charToBoolean(pagePermissions.getDeleteAllowed()))
				.editAllowed(charToBoolean(pagePermissions.getEditAllowed()))
				.launchAllowed(charToBoolean(pagePermissions.getLaunchAllowed())).id(pagePermissions.getId())
	...
	public ResponseStringVO saveApplicationSecurities(SecurityVO applicationSecurityVO) {
		...
		String message = null;
		int count = applicationPermissionsRepository.checkApplicationSecurityExist(
				applicationSecurityVO.getSecurityId(), YoroappsConstants.YES, YorosisContext.get().getTenantId());
		if (count > 0) {
		...
		for (PermissionVO permissionVO : applicationSecurityVO.getPermissionsVOList()) {
			int groupNameCount = yoroGroupsRepository.checkGroupExistOrNot(permissionVO.getGroupId(),
			...
			if (permissionVO.getId() == null) {
				...
				applicationPermissions.setYoroGroups(yoroGroupsRepository
				...
				applicationPermissions.setEditAllowed(booleanToChar(permissionVO.getEditAllowed()));
				applicationPermissions.setLaunchAllowed(booleanToChar(permissionVO.getLaunchAllowed()));
				applicationPermissions.setModifiedBy(YorosisContext.get().getUserName());
				applicationPermissions.setModifiedOn(timestamp);
			}
	...
	public List<PermissionVO> getApplicationPermissionsList(UUID applicationId) {
		...
		for (ApplicationPermissions permissions : applicationPermissionsRepository
				.findByApplicationIdAndTenantIdIgnoreCaseAndActiveFlag(applicationId,
