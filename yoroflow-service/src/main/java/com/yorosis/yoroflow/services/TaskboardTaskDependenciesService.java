...
public class TaskboardTaskDependenciesService {
	...
	public TaskDependenciesVO saveTaskDependencies(TaskDependenciesVO taskDependenciesVO) throws JsonProcessingException {
		...
		TaskboardTask task = taskboardTaskRepository.findByIdAndTenantIdIgnoreCaseAndActiveFlagIgnoreCase(
				taskDependenciesVO.getTaskId(), YorosisContext.get().getTenantId(), YorosisConstants.YES);
		if (taskDependenciesVO.getRemoveDependenciesId() != null
				&& !taskDependenciesVO.getRemoveDependenciesId().isEmpty()) {
	...
	private void removeTaskDependencies(List<UUID> removeDependenciesId) {
		List<TaskboardTaskDependencies> taskboardTaskDependencies = taskboardTaskDependenciesRepository
				.getTaskDependenciesById(removeDependenciesId, YorosisContext.get().getTenantId(),
	...
	public ResponseStringVO removeTaskDepedency(UUID dependencyId) {
		...
		TaskboardTaskDependencies taskboardTaskDependency = taskboardTaskDependenciesRepository
				.findByIdAndTenantIdIgnoreCaseAndActiveFlagIgnoreCase(dependencyId, YorosisContext.get().getTenantId(),
						YorosisConstants.YES);
	...
	private List<TaskboardTaskVO> saveWaitingOnDependencies(TaskDependenciesVO taskDependenciesVO, TaskboardTask task) throws JsonProcessingException {
		...
		for (TaskboardTask taskboardTask : taskboardTaskRepository.getTasksById(waitingOnUUID,
				YorosisContext.get().getTenantId(), YorosisConstants.YES)) {
			Timestamp timestamp = new Timestamp(System.currentTimeMillis());
			TaskboardTaskDependencies taskboardTaskDependencies = TaskboardTaskDependencies.builder()
					.waitingOn(taskboardTask).activeFlag(YorosisConstants.YES)
	...
	private List<TaskboardTaskVO> saveBlockingDependencies(TaskDependenciesVO taskDependenciesVO, TaskboardTask task) throws JsonProcessingException {
		...
		for (TaskboardTask taskboardTask : taskboardTaskRepository.getTasksById(blockingUUID,
				YorosisContext.get().getTenantId(), YorosisConstants.YES)) {
			Timestamp timestamp = new Timestamp(System.currentTimeMillis());
			TaskboardTaskDependencies taskboardTaskDependencies = TaskboardTaskDependencies.builder()
					.blocking(taskboardTask).activeFlag(YorosisConstants.YES)
					.tenantId(YorosisContext.get().getTenantId()).createdBy(YorosisContext.get().getUserName())
					.createdOn(timestamp).modifiedBy(YorosisContext.get().getUserName()).modifiedOn(timestamp)
					.taskboardTask(task).build();
			taskDependencies.add(taskboardTaskDependencies);
	...
	private List<TaskboardTaskVO> saveRelatedTasksDependencies(TaskDependenciesVO taskDependenciesVO,
			TaskboardTask task) throws JsonProcessingException {
		List<TaskboardTaskDependencies> taskDependencies = new ArrayList<>();
		...
		for (TaskboardTask taskboardTask : taskboardTaskRepository.getTasksById(relatedTasksUUID,
				YorosisContext.get().getTenantId(), YorosisConstants.YES)) {
			Timestamp timestamp = new Timestamp(System.currentTimeMillis());
			TaskboardTaskDependencies taskboardTaskDependencies = TaskboardTaskDependencies.builder()
					.relatedTask(taskboardTask).activeFlag(YorosisConstants.YES)
					.tenantId(YorosisContext.get().getTenantId()).createdBy(YorosisContext.get().getUserName())
					.createdOn(timestamp).modifiedBy(YorosisContext.get().getUserName()).modifiedOn(timestamp)
					.taskboardTask(task).build();
			taskDependencies.add(taskboardTaskDependencies);
	...
	private TaskboardTaskVO constructEntityToVO(TaskboardTask task, TaskboardTaskDependencies taskDependencies) {
		return TaskboardTaskVO.builder().id(task.getId()).taskId(task.getTaskId())
				.dependencyId(taskDependencies.getId()).taskName(task.getTaskName()).build();
	}
	...
	public TaskDependenciesVO getTaskDependencies(UUID taskId) {
		...
		List<TaskboardTaskDependencies> taskDependencies = taskboardTaskDependenciesRepository
				.getDependenciesByTaskId(taskId, YorosisContext.get().getTenantId(), YorosisConstants.YES);
