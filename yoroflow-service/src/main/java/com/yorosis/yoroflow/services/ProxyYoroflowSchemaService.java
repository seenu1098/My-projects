...
public class ProxyYoroflowSchemaService {
	...
	public List<WorkflowTemplatesVO> getWorkflowTemplates() {
		YorosisContext context = setYoroflowContext();
		try {
			return workflowTemplatesService.getWorkflowTemplates();
		} finally {
			clearYoroflowContext(context);
		}
	}

	public void setUpYoroContext(final String tenantId, final String authorizationCode, final UUID appId,
			final UUID taskboardId) throws YorosisException {
		YorosisContext context = setYoroflowContext();
		Customers customer = customerService.getCustomerByTenant(tenantId);
		if (customer != null) {
			context = YorosisContext.builder().tenantId(customer.getTenantId()).userName(customer.getCreatedBy())
					.build();
	...
	public List<WorkflowSummaryVO> getAllWorkflowNameList(UUID workspaceId, PaginationVO vo, String tenantId) {
		YorosisContext context = setYoroflowContext();
		Customers customer = customerService.getCustomerByTenant(tenantId);
		if (customer != null) {
			YorosisContext.set(YorosisContext.builder().tenantId(customer.getTenantId()).build());
			try {
				return workspaceService.getAllWorkflowNameList(workspaceId, vo);
			} finally {
				clearYoroflowContext(context);
			}
		}
