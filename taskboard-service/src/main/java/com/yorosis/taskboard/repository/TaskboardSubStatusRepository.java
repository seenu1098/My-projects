...
public interface TaskboardSubStatusRepository extends JpaRepository<TaskboardSubStatus, UUID> {
	...
	public List<TaskboardSubStatus> getSubStatusList(@Param("taskboardColumnId") TaskboardColumns taskboardColumnId,
			@Param("tenantId") String tenantId, @Param("activeFlag") String activeFlag);

	public TaskboardSubStatus findByIdAndTenantIdIgnoreCaseAndActiveFlagIgnoreCase(UUID id, String tenantId,
			String activeFlag);

	@Query("select p from TaskboardSubStatus p where p.id in :id and p.tenantId = :tenantId and p.activeFlag = :activeFlag")
	public List<TaskboardSubStatus> getIdAndTenantIdIgnoreCaseAndActiveFlagIgnoreCase(@Param("id") List<UUID> id,
			@Param("tenantId") String tenantId, @Param("activeFlag") String activeFlag);
}
