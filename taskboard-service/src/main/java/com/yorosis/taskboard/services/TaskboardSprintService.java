...
import com.yorosis.taskboard.models.PaginationVO;
import com.yorosis.taskboard.models.ResponseStringVO;
import com.yorosis.taskboard.models.TaskboardTaskVO;
import com.yorosis.taskboard.models.sprint.SaveSprintVo;
import com.yorosis.taskboard.models.sprint.SprintSettingsVo;
import com.yorosis.taskboard.models.sprint.SprintTasksVo;
import com.yorosis.taskboard.models.sprint.SprintsVO;
import com.yorosis.taskboard.models.sprint.WorkLogListVo;
import com.yorosis.taskboard.models.sprint.WorkLogVo;
...
import com.yorosis.taskboard.repository.TaskboardSprintTasksRepository;
import com.yorosis.taskboard.repository.TaskboardSprintWorkLogRepository;
import com.yorosis.taskboard.repository.TaskboardSprintsRepository;
...
public class TaskboardSprintService {
	...
	private TaskboardSprints constructSprintSettingsVoToDto(SprintsVO sprintsVO) {
		...
		return TaskboardSprints.builder().sprintStatus("p").activeFlag(YorosisConstants.YES).tenantId(YorosisContext.get().getTenantId())
				.createdBy(YorosisContext.get().getUserName()).modifiedBy(YorosisContext.get().getUserName()).createdOn(timestamp).modifiedOn(timestamp)
				.build();
	}

	public TaskboardSprintSettings constructSprintSettingsVOToDto(SprintSettingsVo sprintSettingsVo) {
		Timestamp timestamp = new Timestamp(System.currentTimeMillis());
		return TaskboardSprintSettings.builder().sprintDuration(sprintSettingsVo.getSprintDuration()).activeFlag(YorosisConstants.YES)
				.tenantId(YorosisContext.get().getTenantId()).createdBy(YorosisContext.get().getUserName()).modifiedBy(YorosisContext.get().getUserName())
				.createdOn(timestamp).modifiedOn(timestamp).sprintStartDay(sprintSettingsVo.getSprintStartDay())
				.sprintDurationType(sprintSettingsVo.getSprintDurationType()).sprintEstimations(sprintSettingsVo.getSprintEstimations()).build();
	}
	...
	private SprintSettingsVo constructSprintSettingsDtoToVo(TaskboardSprintSettings taskboardSprintSettings) {
		return SprintSettingsVo.builder().sprintSettingsId(taskboardSprintSettings.getSprintSettingId())
				.sprintDuration(taskboardSprintSettings.getSprintDuration()).sprintStartDay(taskboardSprintSettings.getSprintStartDay())
				.sprintEstimations(taskboardSprintSettings.getSprintEstimations()).sprintDurationType(taskboardSprintSettings.getSprintDurationType())
	...
	private SprintsVO constructSprintsDtoToVo(TaskboardSprints taskboardSettings) {
		return SprintsVO.builder().sprintId(taskboardSettings.getSprintId()).sprintSeqNumber(taskboardSettings.getSprintSeqNumber())
				...
				.sprintTotalOriginalPoints(taskboardSettings.getSprintTotalOriginalPoints()).sprintEndDate(taskboardSettings.getSprintEndDate())
				.sprintStatus(taskboardSettings.getSprintStatus()).sprintStartDate(taskboardSettings.getSprintStartDate())
				.sprintName(taskboardSettings.getSprintName()).build();
	}

	private TaskboardSprintWorkLog constructWorkLogVoToDto(WorkLogVo workLogVo) {
		Timestamp timestamp = new Timestamp(System.currentTimeMillis());
		return TaskboardSprintWorkLog.builder().description(workLogVo.getDescription()).workDate(workLogVo.getWorkDate()).timespent(workLogVo.getTimespent())
	...
	private WorkLogVo constructWorkLogDtoToVo(TaskboardSprintWorkLog taskboardSprintWorkLog) {
		return WorkLogVo.builder().description(taskboardSprintWorkLog.getDescription()).workDate(taskboardSprintWorkLog.getWorkDate())
				.timespent(taskboardSprintWorkLog.getTimespent()).userId(taskboardSprintWorkLog.getUser().getUserId())
				.workLogId(taskboardSprintWorkLog.getWorkLogId()).loggedatTime(taskboardSprintWorkLog.getCreatedOn())
	...
	public ResponseStringVO saveAndUpdateSprintSetting(SprintSettingsVo sprintSettingsVo, Taskboard taskboard) {
		if (sprintSettingsVo.getSprintSettingsId() == null) {
			if (taskboard == null) {
				taskboard = taskboardRepository.findByIdAndTenantIdIgnoreCaseAndActiveFlagIgnoreCase(sprintSettingsVo.getTaskboardId(),
						YorosisContext.get().getTenantId(), YorosisConstants.YES);
			}
			if (taskboard != null) {
				if (taskboardSprintSettingsRepository.getTaskboardSprintSettingsCountByTaskboardId(sprintSettingsVo.getTaskboardId(),
						YorosisContext.get().getTenantId(), YorosisConstants.YES) > 0) {
				...
				return ResponseStringVO.builder().response("Sprint Saved Successfully").id(taskboardSprintSettings.getSprintSettingId()).build();
			} else {
			...
			TaskboardSprintSettings taskboardSprintSettings = taskboardSprintSettingsRepository
					.getTaskboardSprintSettingsById(sprintSettingsVo.getSprintSettingsId(), YorosisContext.get().getTenantId(), YorosisConstants.YES);
	...
	public ResponseStringVO deleteSprintSettings(UUID sprintSettingsId) {
		TaskboardSprintSettings taskboardSprintSettings = taskboardSprintSettingsRepository.getTaskboardSprintSettingsById(sprintSettingsId,
				YorosisContext.get().getTenantId(), YorosisConstants.YES);
		if (taskboardSprintSettings != null) {
	...
	public ResponseStringVO addEstimateHoursForTask(TaskboardTaskVO vo) {
		TaskboardTask taskboardTask = taskboardTaskRepository.findByIdAndTenantIdIgnoreCaseAndActiveFlagIgnoreCase(vo.getId(),
				YorosisContext.get().getTenantId(), YorosisConstants.YES);
		if (taskboardTask != null && StringUtils.equals(taskboardTask.getTaskboard().getSprintEnabled(), YorosisConstants.YES)) {
	...
	private void updateOriginalAndEstimateHoursForTask(TaskboardTask taskboardTask, String type, Float hours, Integer points) {
		List<TaskboardSprints> taskboardSprintsList = taskboardSprintsRepository.getTaskboardSprintsListByTaskboard(taskboardTask.getTaskboard().getId(),
				YorosisContext.get().getTenantId(), YorosisConstants.YES);
		...
		if (taskboardSprintsList != null && !taskboardSprintsList.isEmpty()) {
			for (TaskboardSprints taskboardSprints : taskboardSprintsList) {
				if (StringUtils.equals(taskboardSprints.getSprintStatus(), "p")) {
					TaskboardSprintTask sprintTask = taskboardSprintTasksRepository.getTaskboardSprintTasksById(taskboardTask.getId(),
							taskboardSprints.getSprintId(), YorosisContext.get().getTenantId(), YorosisConstants.YES);
					if (sprintTask != null) {
						if (StringUtils.equals(type, "original")) {
							Integer totalSprintEstimatePoints = taskboardSprints.getSprintTotalOriginalPoints() == null ? 0
							...
							totalSprintEstimatePoints = (totalSprintEstimatePoints
									+ (taskboardTask.getOriginalPoints() == null ? 0 : taskboardTask.getOriginalPoints())) - (points == null ? 0 : points);
						...
						if (StringUtils.equals(type, "estimate")) {
							...
							totalSprintEstimateHours = (totalSprintEstimateHours
									+ (taskboardTask.getEstimateHours() == null ? 0 : taskboardTask.getEstimateHours())) - (hours == null ? 0.0 : hours);
	...
	public SaveSprintVo saveAndUpdateSprint(SprintsVO sprintsVo) {
		if (sprintsVo != null) {
			if (sprintsVo.getSprintId() == null) {
				TaskboardSprintSettings taskboardSprintSettings = taskboardSprintSettingsRepository
						.getTaskboardSprintSettingsById(sprintsVo.getSprintSettingsId(), YorosisContext.get().getTenantId(), YorosisConstants.YES);
				if (taskboardSprintSettings != null) {
//					TaskboardSprints taskboardSprints = constructSprintSettingsVoToDto(sprintsVo);
					...
					if (StringUtils.equals(taskboardSprintSettings.getSprintDurationType(), "w")) {
						endDate = startDate.plusWeeks(taskboardSprintSettings.getSprintDuration() * sprintsVo.getSprintCounts());
						endDate = endDate.minusDays(1);
					} else {
						endDate = startDate.plusDays((taskboardSprintSettings.getSprintDuration() * sprintsVo.getSprintCounts()) - 1);
					}
					int taskboardSprintSettingsCountBetweenDays = taskboardSprintsRepository.getTaskboardSprintsCountByStartAndEndDate(
							taskboardSprintSettings.getSprintSettingId(), YorosisContext.get().getTenantId(), YorosisConstants.YES,
							Timestamp.valueOf(startDate), Timestamp.valueOf(endDate));
					if (taskboardSprintSettingsCountBetweenDays == 0) {
						int taskboardSprintSettingsCount = taskboardSprintsRepository.getTaskboardSprintsCountByTaskboard(
								taskboardSprintSettings.getSprintSettingId(), YorosisContext.get().getTenantId(), YorosisConstants.YES);
						...
						while (i <= sprintsVo.getSprintCounts()) {
							...
							if (i == 1) {
								...
								if (StringUtils.equals(taskboardSprintSettings.getSprintDurationType(), "w")) {
									startDate = startDate.plusWeeks(taskboardSprintSettings.getSprintDuration() * (i - 1));
									...
									startDate = startDate.plusDays(taskboardSprintSettings.getSprintDuration() * (i - 1));
									taskboardSprints.setSprintStartDate(Timestamp.valueOf(startDate));
									endDate = startDate.plusDays(taskboardSprintSettings.getSprintDuration() - 1);
									taskboardSprints.setSprintEndDate(Timestamp.valueOf(endDate));
								}
						...
						return SaveSprintVo.builder().response("Sprint Saved Successfully")
								.sprintVoList(getSprintBySprintSettings(taskboardSprintSettings.getSprintSettingId())).build();
					} else {
						return SaveSprintVo.builder()
								.response(
				...
				TaskboardSprints taskboardSprints = taskboardSprintsRepository.getTaskboardSprintsById(sprintsVo.getSprintId(),
						YorosisContext.get().getTenantId(), YorosisConstants.YES);
...
//				taskboardSprints.setSprintEndDate(sprintsVo.getSprintEndDate());
					...
					return SaveSprintVo.builder().response("Sprint Updated Successfully")
							.sprintVoList(getSprintBySprintSettings(taskboardSprints.getTaskboardSprintSettings().getSprintSettingId())).build();
				}
			}
		}
		return null;
	...
	public void deleteSprintByTaskboard(UUID taskboardId) {
		List<TaskboardSprints> taskboardSprintsList = taskboardSprintsRepository.getTaskboardSprintsListByTaskboardAndRunning(taskboardId,
				YorosisContext.get().getTenantId(), YorosisConstants.YES);
		if (taskboardSprintsList != null && !taskboardSprintsList.isEmpty()) {
	...
	public ResponseStringVO deleteSprint(UUID sprintId) {
		TaskboardSprints taskboardSprints = taskboardSprintsRepository.getTaskboardSprintsById(sprintId, YorosisContext.get().getTenantId(),
		...
		return ResponseStringVO.builder()
				.response(taskboardSprints == null ? "Sprint does not exist"
						: StringUtils.equals(taskboardSprints.getSprintStatus(), "c") ? "Completed Sprint will not be deleted"
								: StringUtils.equals(taskboardSprints.getSprintStatus(), "r") ? "Running Sprint will not be deleted" : "Sprint not deleted")
				.build();
	}
	...
	public ResponseStringVO checkCompleteSprint(UUID sprintId, UUID taskboardId) {
		List<UUID> sprintTaskIdList = taskboardSprintTasksRepository.getTaskboardSprintTasksBySprintId(sprintId, YorosisContext.get().getTenantId(),
				YorosisConstants.YES);
		if (sprintTaskIdList != null && !sprintTaskIdList.isEmpty()) {
			List<String> taskIdList = taskboardTaskRepository.checkAllAndGetTaskIdTasksInDone(sprintTaskIdList, YorosisContext.get().getTenantId(),
					YorosisConstants.YES, taskboardId);
	...
	public List<SprintsVO> getSprintByTaskBoard(UUID taskboardId) {
		...
		List<TaskboardSprints> taskboardSprintsList = taskboardSprintsRepository.getTaskboardSprintsListByTaskboard(taskboardId,
				YorosisContext.get().getTenantId(), YorosisConstants.YES);
		if (taskboardSprintsList != null && !taskboardSprintsList.isEmpty()) {
			SprintSettingsVoList = taskboardSprintsList.stream().map(this::constructSprintsDtoToVo).collect(Collectors.toList());
		}
	...
	public List<SprintsVO> getSprintBySprintSettings(UUID sprintSettingId) {
		List<SprintsVO> SprintSettingsVoList = new ArrayList<>();
		List<TaskboardSprints> taskboardSprintsList = taskboardSprintsRepository.getTaskboardSprintsListBySprintSettings(sprintSettingId,
				YorosisContext.get().getTenantId(), YorosisConstants.YES);
		if (taskboardSprintsList != null && !taskboardSprintsList.isEmpty()) {
			SprintSettingsVoList = taskboardSprintsList.stream().map(this::constructSprintsDtoToVo).collect(Collectors.toList());
		}
	...
	public List<SprintsVO> getSprintWithInPreparationByTaskboard(UUID taskboardId) {
		List<SprintsVO> SprintSettingsVoList = new ArrayList<>();
		List<TaskboardSprints> taskboardSprintsList = taskboardSprintsRepository.getTaskboardSprintsInPreparationListByTaskboard(taskboardId,
				YorosisContext.get().getTenantId(), YorosisConstants.YES);
		if (taskboardSprintsList != null && !taskboardSprintsList.isEmpty()) {
			SprintSettingsVoList = taskboardSprintsList.stream().map(this::constructSprintsDtoToVo).collect(Collectors.toList());
		}
	...
	public SprintSettingsVo getSprintSettingsByTaskBoard(UUID taskboardId) {
		...
		TaskboardSprintSettings taskboardSprintSettings = taskboardSprintSettingsRepository.getTaskboardSprintSettingsByTaskboardId(taskboardId,
				YorosisContext.get().getTenantId(), YorosisConstants.YES);
		if (taskboardSprintSettings != null) {
	...
	public ResponseStringVO startSprint(UUID sprintId) {
		TaskboardSprints taskboardSprints = taskboardSprintsRepository.getTaskboardSprintsById(sprintId, YorosisContext.get().getTenantId(),
				YorosisConstants.YES);
		if (taskboardSprints != null) {
			if (taskboardSprintsRepository.checkStartTaskboardSprintsById(sprintId, YorosisContext.get().getTenantId(), YorosisConstants.YES) == 0) {
				if (checkStartDate(taskboardSprints.getSprintStartDate()) && StringUtils.equals(taskboardSprints.getSprintStatus(), "p")) {
//				saveTaskEstimations(taskboardSprints, sprintId);
					...
					return ResponseStringVO.builder()
							.response(!StringUtils.equals(taskboardSprints.getSprintStatus(), "p") ? "Sprint Already Started" : "Sprint Not Started").build();
				}
			} else {
	...
	public ResponseStringVO checkStartSprint(UUID sprintId) {
		TaskboardSprints taskboardSprints = taskboardSprintsRepository.getTaskboardSprintsById(sprintId, YorosisContext.get().getTenantId(),
				YorosisConstants.YES);
		if (taskboardSprints != null) {
			return ResponseStringVO.builder().response(
					StringUtils.equals(taskboardSprints.getSprintStatus(), "p") ? "p" : StringUtils.equals(taskboardSprints.getSprintStatus(), "r") ? "r" : "c")
					.build();
		}
		return ResponseStringVO.builder().response("invalid").build();
	}

	private TaskboardSprintTask constructTaskboardSprintTaskVoToDto(SprintTasksVo sprintTasksVo, TaskboardSprints taskboardSprints) {
		Timestamp timestamp = new Timestamp(System.currentTimeMillis());
		return TaskboardSprintTask.builder().activeFlag(YorosisConstants.YES).tenantId(YorosisContext.get().getTenantId())
				.createdBy(YorosisContext.get().getUserName()).modifiedBy(YorosisContext.get().getUserName()).createdOn(timestamp).modifiedOn(timestamp)
	...
	public ResponseStringVO saveAndUpdateSprintTasks(SprintTasksVo sprintTasksVo) {
		TaskboardSprints taskboardSprints = taskboardSprintsRepository.getTaskboardSprintsById(sprintTasksVo.getSprintId(), YorosisContext.get().getTenantId(),
				YorosisConstants.YES);
		if (taskboardSprints != null) {
			List<TaskboardSprintTask> taskboardSprintTaskList = new ArrayList<>();
			int totalSprintEstimatePoints = taskboardSprints.getSprintTotalOriginalPoints() == null ? 0 : taskboardSprints.getSprintTotalOriginalPoints();
			Double totalSprintEstimateHours = taskboardSprints.getSprintTotalEstimatedHours() == null ? 0.0 : taskboardSprints.getSprintTotalEstimatedHours();
			List<TaskboardTask> taskboardTaskList = taskboardTaskRepository.getTasksById(sprintTasksVo.getTaskboardTaskId(), YorosisContext.get().getTenantId(),
					YorosisConstants.YES);
			if (taskboardTaskList != null && !taskboardTaskList.isEmpty()) {
				List<String> taskIdList = new ArrayList<>();
				for (TaskboardTask t : taskboardTaskList) {
					List<UUID> sprintTaskIdList = taskboardSprintTasksRepository.getTaskboardSprintTasksBySprintId(sprintTasksVo.getSprintId(),
							YorosisContext.get().getTenantId(), YorosisConstants.YES);
					if (sprintTaskIdList == null || !sprintTaskIdList.contains(t.getId())) {
						TaskboardSprintTask taskboardSprintTask = constructTaskboardSprintTaskVoToDto(sprintTasksVo, taskboardSprints);
...
//					if (StringUtils.equals(taskboardSprints.getSprintStatus(), "r")) {
						...
						totalSprintEstimatePoints = totalSprintEstimatePoints + (t.getOriginalPoints() == null ? 0 : t.getOriginalPoints());
						totalSprintEstimateHours = totalSprintEstimateHours + (t.getEstimateHours() == null ? 0 : t.getEstimateHours());
//					}
				...
				return ResponseStringVO.builder()
						.response(taskIdList.isEmpty() ? "Sprint Task Saved Successfully" : taskIdList.toString() + " already saved to this sprint").build();
			}
	...
	public ResponseStringVO deleteSprintTask(SprintTasksVo sprintTasksVo) {
		TaskboardSprintTask taskboardSprintTask = taskboardSprintTasksRepository.getTaskboardSprintTasksById(sprintTasksVo.getSprintTaskId(),
	...
	public ResponseStringVO saveWorklog(WorkLogVo workLogVo) {
		User user = userRepository.findByUserNameIgnoreCaseAndTenantIdIgnoreCaseAndActiveFlagIgnoreCase(YorosisContext.get().getUserName(),
				YorosisContext.get().getTenantId(), YorosisConstants.YES);
		TaskboardSprintTask taskboardSprintTask = taskboardSprintTasksRepository.getTaskboardSprintTasksById(workLogVo.getSprintTaskId(),
				YorosisContext.get().getTenantId(), YorosisConstants.YES);
		if (workLogVo.getWorkLogId() == null) {
			...
			TaskboardSprintWorkLog taskboardSprintWorkLog = taskboardSprintWorkLogRepository.getTaskboardSprintWorkLogById(workLogVo.getWorkLogId(),
					YorosisContext.get().getTenantId(), YorosisConstants.YES);
			if (taskboardSprintWorkLog != null && taskboardSprintTask != null && StringUtils.equals(user.getUserName(), YorosisContext.get().getUserName())) {
	...
	private void saveTaskWorkLog(TaskboardSprintTask taskboardSprintTask, WorkLogVo workLogVo, TaskboardSprintWorkLog taskboardSprintWorkLog) {
		Float totalHoursSpent = taskboardSprintTask.getSprintTotalHoursSpent() == null ? 0 : taskboardSprintTask.getSprintTotalHoursSpent();
	...
	public void getRemainingHours(TaskboardTaskVO taskVO, UUID sprintId) {
		TaskboardSprintTask taskboardSprintTask = taskboardSprintTasksRepository.getTaskboardSprintTasksById(taskVO.getId(), sprintId,
				YorosisContext.get().getTenantId(), YorosisConstants.YES);
		if (taskboardSprintTask != null) {
			...
			if (taskboardSprintTask.getSprintEstimatedHours() != null) {
				if (taskboardSprintTask.getSprintTotalHoursSpent() == null) {
					...
					Float remainingHours = taskboardSprintTask.getSprintEstimatedHours() - taskboardSprintTask.getSprintTotalHoursSpent();
	...
	public TaskboardTaskVO getRemainingHours(UUID sprintTaskId) {
		TaskboardSprintTask taskboardSprintTask = taskboardSprintTasksRepository.getTaskboardSprintTasksById(sprintTaskId, YorosisContext.get().getTenantId(),
				YorosisConstants.YES);
		if (taskboardSprintTask != null) {
			if (taskboardSprintTask.getSprintTotalHoursSpent() == null || taskboardSprintTask.getSprintEstimatedHours() == null) {
	...
	public WorkLogListVo getWorklogList(PaginationVO vo) {
		...
		Integer count = 0;
		List<TaskboardSprintWorkLog> taskboardSprintWorkLogList = taskboardSprintWorkLogRepository.getTaskboardSprintWorkLogByTaskId(vo.getTaskboardId(),
				YorosisContext.get().getTenantId(), YorosisConstants.YES, pageable);
		count = taskboardSprintWorkLogRepository.getTaskboardSprintWorkLogCountByTaskId(vo.getTaskboardId(), YorosisContext.get().getTenantId(),
				YorosisConstants.YES);
		if (taskboardSprintWorkLogList != null && !taskboardSprintWorkLogList.isEmpty()) {
			workLogVoList = taskboardSprintWorkLogList.stream().map(this::constructWorkLogDtoToVo).collect(Collectors.toList());
		}
		return WorkLogListVo.builder().taskId(vo.getTaskboardId()).totalRecords(count).workLogVoList(workLogVoList).build();
	}
