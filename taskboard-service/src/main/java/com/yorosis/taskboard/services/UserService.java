...
import com.yorosis.taskboard.repository.MessageHistoryRepository;
import com.yorosis.taskboard.repository.RoleRepository;
...
public class UserService implements UserDetailsService {
	...
	private UsersVO constructDTOToVO(User user) {
		...
		return UsersVO.builder().userId(user.getUserId()).firstName(user.getFirstName()).lastName(user.getLastName()).userName(user.getUserName())
				.emailId(user.getEmailId()).contactEmailId(user.getContactEmailId()).groupVOList(groupVOList).color(user.getColor()).build();
	}

	private User constructDTOToVO(UsersVO userVO) {
		String hashedPassword = bcryptEncoder.encode(userVO.getPassword());
		return User.builder().userId(userVO.getUserId()).firstName(userVO.getFirstName()).lastName(userVO.getLastName()).userName(userVO.getUserName())
				.userPassword(hashedPassword).emailId(userVO.getEmailId()).contactEmailId(userVO.getContactEmailId())
				.createdBy(YorosisContext.get().getUserName()).activeFlag(YorosisConstants.YES).build();
	}
	...
	public ResponseStringVO createAndUpdateUsers(UsersVO userVO) {
		...
		if (userVO.getUserId() == null) {
			...
			for (GroupVO group : userVO.getGroupVOList()) {
				userGroups.add(UserGroup.builder().group(groupRepository.getOne(group.getGroupId())).user(user).updatedDate(timestamp)
						.createdBy(YorosisContext.get().getUserName()).updatedBy(YorosisContext.get().getUserName()).createdDate(timestamp).build());
			}
	...
	public List<UsersVO> getUserName(String name) {
		...
		for (User code : list) {
			if (!StringUtils.equalsAnyIgnoreCase(YorosisContext.get().getUserName(), code.getUserName())) {
				UsersVO vo = UsersVO.builder().firstName(code.getFirstName()).lastName(code.getLastName()).userId(code.getUserId())
						.userName(code.getFirstName() + " " + code.getLastName()).build();
				usersList.add(vo);
			}
		}
	...
	public ResponseStringVO save(UsersVO uservo) {
		...
		if (uservo.getUserId() == null) {
			...
			int userCount = userRepository.findByEmailIdCount(uservo.getEmailId(), YorosisConstants.YES, YorosisContext.get().getTenantId());
			...
			if (userCount == 0) {
				...
				Timestamp timestamp = new Timestamp(System.currentTimeMillis());
				User user = User.builder().userName(uservo.getEmailId()).firstName(uservo.getFirstName()).lastName(uservo.getLastName())
						.userPassword(uservo.getPassword()).emailId(uservo.getEmailId()).activeFlag(YorosisConstants.YES)
				...
				return ResponseStringVO.builder().response(String.format("User %s created successfully", user.getUserName())).build();
			} else {
				return ResponseStringVO.builder().response(String.format("User %s already exists", uservo.getUserName())).build();
			}
		} else {
			Timestamp timestamp = new Timestamp(System.currentTimeMillis());
			...
			return ResponseStringVO.builder().response(String.format("User %s updated successfully", user.getUserName())).build();
		}
	}
	...
	public List<UsersVO> getMessageHistoryUsers() {
		...
		for (User user : userRepository.getAllUsers(YorosisContext.get().getTenantId(), YorosisConstants.YES)) {
			UsersVO userVO = constructDTOToVO(user);
			long count = messageHistoryRepository.getUnReadMessageCount(userRepository.findByUserName(YorosisContext.get().getUserName()).getUserId(),
					user.getUserId(), YorosisContext.get().getTenantId());
	...
	public UsersVO getUsersForm(UUID id) {
		...
		List<UserGroup> groups = userGroupRepository.getUserGroups(id, YorosisContext.get().getTenantId(), YorosisConstants.YES);
		...
		return UsersVO.builder().userId(user.getUserId()).emailId(user.getEmailId()).contactEmailId(user.getContactEmailId()).firstName(user.getFirstName())
				.lastName(user.getLastName()).password(user.getUserPassword()).userType(user.getUserType()).profilePicture(user.getProfilePicture())
