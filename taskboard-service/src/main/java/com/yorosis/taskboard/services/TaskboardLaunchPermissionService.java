...
import com.yorosis.taskboard.models.LaunchPermissionVo;
import com.yorosis.taskboard.models.LaunchTaskListVo;
import com.yorosis.taskboard.models.LaunchTaskboardTaskVo;
...
import com.yorosis.taskboard.repository.TaskboardLaunchPermissionRepository;
...
public class TaskboardLaunchPermissionService {
	...
	private TaskboardLaunchPermission constructTaskboardLaunchPermissionDtotoVo(LaunchPermissionVo launchPermissionVo) {
		...
		return TaskboardLaunchPermission.builder().activeFlag(YorosisConstants.YES).tenantId(YorosisContext.get().getTenantId())
				...
				.allowTaskboardTeams(booleanToChar(launchPermissionVo.getAllowTaskboardTeams())).createdBy(YorosisContext.get().getUserName())
				.createdOn(currentTimestamp).modifiedBy(YorosisContext.get().getUserName()).modifiedOn(currentTimestamp).build();
	}

	private LaunchPermissionVo constructTaskboardLaunchPermissionDtotoVo(TaskboardLaunchPermission taskboardLaunchPermission) {
	...
	public ResponseStringVO saveAndUpdateTaskboardLaunchPermission(LaunchPermissionVo launchPermissionVo) {
		...
		if (launchPermissionVo != null) {
			TaskboardLaunchPermission taskboardLaunchPermission = constructTaskboardLaunchPermissionDtotoVo(launchPermissionVo);
			List<TaskboardLaunchPermission> taskboardLaunchPermissionList = new ArrayList<>();
			Taskboard taskboard = taskboardRepository.findByIdAndTenantIdIgnoreCaseAndActiveFlagIgnoreCase(launchPermissionVo.getTaskboardId(),
			...
			if (taskboard != null) {
				List<TaskboardLaunchPermission> taskboardLaunchPermissionListByTaskboard = taskboardLaunchPermissionRepository
						.getTaskboardLaunchPermissionListByTenantIdIgnoreCaseAndActiveFlagIgnoreCase(YorosisContext.get().getTenantId(), YorosisConstants.YES,
				...
				if ((launchPermissionVo.getAllowLoggedInUser() != null && BooleanUtils.isTrue(launchPermissionVo.getAllowLoggedInUser()))
						|| (launchPermissionVo.getAllowWorkspaceUsers() != null && BooleanUtils.isTrue(launchPermissionVo.getAllowWorkspaceUsers()))
						|| (launchPermissionVo.getAllowTaskboardUser() != null && BooleanUtils.isTrue(launchPermissionVo.getAllowTaskboardUser()))
						|| (launchPermissionVo.getAllowTaskboardTeams() != null && BooleanUtils.isTrue(launchPermissionVo.getAllowTaskboardTeams()))) {
					taskboardLaunchPermission.setTaskboard(taskboard);
					taskboardLaunchPermissionList.add(taskboardLaunchPermission);
				}
				if (launchPermissionVo.getAllowTeamsList() != null && !launchPermissionVo.getAllowTeamsList().isEmpty()) {
					List<Group> groupList = groupRepository.getGroupListByTenantIdIgnoreCaseAndActiveFlagIgnoreCase(YorosisContext.get().getTenantId(),
							YorosisConstants.YES, launchPermissionVo.getAllowTeamsList());
					if (groupList != null && !groupList.isEmpty()) {
						groupList.stream().forEach(g -> {
							TaskboardLaunchPermission launchPermission = constructTaskboardLaunchPermissionDtotoVo(launchPermissionVo);
				...
				if (launchPermissionVo.getAllowUsersList() != null && !launchPermissionVo.getAllowUsersList().isEmpty()) {
					List<User> userList = usersRepository.findUsersByID(launchPermissionVo.getAllowUsersList(), YorosisContext.get().getTenantId());
	...
	public LaunchPermissionVo getTaskboardLaunchPermission(UUID taskboardId) {
		List<TaskboardLaunchPermission> taskboardLaunchPermissionListByTaskboard = taskboardLaunchPermissionRepository
				.getTaskboardLaunchPermissionListByTenantIdIgnoreCaseAndActiveFlagIgnoreCase(YorosisContext.get().getTenantId(), YorosisConstants.YES,
						taskboardId);
		if (taskboardLaunchPermissionListByTaskboard != null && !taskboardLaunchPermissionListByTaskboard.isEmpty()) {
			LaunchPermissionVo LaunchPermissionVo = constructTaskboardLaunchPermissionDtotoVo(taskboardLaunchPermissionListByTaskboard.get(0));
	...
	public List<TaskboardVO> getTaskboardByLaunch(String workspaceId) {
		...
		List<Taskboard> taskboardList = taskboardRepository.getTaskboardListByLaunch(userVO.getUserId(), userGroupIdsList, getWorkspace(workspaceId),
				YorosisContext.get().getTenantId(), YorosisConstants.YES);
	...
	private List<UUID> getAllWorkpace() {
		UsersVO userVO = userService.getLoggedInUserDetails();
		List<UUID> userGroupIdsList = getGroupAsUUID(userVO);
		return workspaceRepository.getListBasedonTenantIdAndActiveFlag(YorosisContext.get().getTenantId(), YorosisConstants.YES, userVO.getUserId(),
				userGroupIdsList);
	}
	...
	private TaskboardVO loadTaskBoardVo(Taskboard taskboard) {
		TaskboardColumns taskboardColumns = taskboard.getTaskboardColumns().stream().filter(c -> StringUtils.equals(String.valueOf(c.getColumnOrder()), "0"))
				.collect(Collectors.toList()).get(0);

		return TaskboardVO.builder().name(taskboard.getName()).description(taskboard.getDescription()).taskName(taskboard.getTaskName())
				.taskboardKey(taskboard.getTaskboardKey()).id(taskboard.getId()).generatedTaskId(taskboard.getGeneratedTaskId())
				.startColumn(taskboardColumns.getColumnName()).formId(taskboardColumns.getFormId()).version(taskboardColumns.getVersion())
				.sprintEnabled(charToBoolean(taskboard.getSprintEnabled())).launchButtonName(taskboard.getLaunchButtonName()).build();
	}
	...
	public LaunchTaskboardTaskVo getTaskboardTaskListByLaunch(PaginationVO vo, UUID workspaceId) {
		...
		List<TaskboardTask> taskboardTaskList = taskboardTaskRepository.getTaskboardTasksForLaunch(YorosisContext.get().getUserName(), workspaceIdList,
				YorosisContext.get().getTenantId(), YorosisConstants.YES, pageable);
		count = taskboardTaskRepository.getTaskboardTasksForLaunchCount(YorosisContext.get().getUserName(), workspaceIdList, YorosisContext.get().getTenantId(),
				YorosisConstants.YES);
		if (taskboardTaskList != null && !taskboardTaskList.isEmpty()) {
			taskboardTaskListVo = taskboardTaskList.stream().map(this::constructTaskboardTaskVo).collect(Collectors.toList());
		}
	...
	private LaunchTaskListVo constructTaskboardTaskVo(TaskboardTask taskboardTask) {
		return LaunchTaskListVo.builder().reqName(taskboardTask.getTaskName()).status(taskboardTask.getStatus())
				.submittedDate(taskboardTask.getCreatedOn().toLocalDateTime()).taskboardTaskId(taskboardTask.getId()).build();
	}
