...
import com.yorosis.taskboard.models.TaskboardSubStatusVO;
...
public class TaskboardSubStatusService {
	...
	private TaskboardSubStatus constructSubStatusVoToDto(SubStatusVO subStatus, TaskboardColumns column) {
		...
		return TaskboardSubStatus.builder().subStatusName(subStatus.getName()).subStatusColor(subStatus.getColor()).activeFlag(YorosisConstants.YES)
				.tenantId(YorosisContext.get().getTenantId()).taskboardColumnId(column).createdBy(YorosisContext.get().getUserName())
				.modifiedBy(YorosisContext.get().getUserName()).createdOn(timestamp).modifiedOn(timestamp).columnOrder(subStatus.getColumnOrder()).build();
	}

	private SubStatusVO constructSubStatusDtoToVo(TaskboardSubStatus taskboardSubStatus) {
		return SubStatusVO.builder().id(taskboardSubStatus.getId()).name(taskboardSubStatus.getSubStatusName()).color(taskboardSubStatus.getSubStatusColor())
				.columnOrder(taskboardSubStatus.getColumnOrder()).previousName(taskboardSubStatus.getSubStatusName()).build();
	}
	...
	public List<SubStatusVO> saveSubStatus(TaskboardSubStatusVO taskboardSubStatusVO) {
		...
		TaskboardColumns column = taskboardColumnsRepository.findByIdAndTenantIdIgnoreCaseAndActiveFlagIgnoreCase(taskboardSubStatusVO.getTaskboardColumnId(),
		...
		for (SubStatusVO subStatusVO : taskboardSubStatusVO.getSubStatus()) {
			...
			if (subStatusVO.getId() == null) {
				...
				subStatus = taskboardSubStatusRepository.findByIdAndTenantIdIgnoreCaseAndActiveFlagIgnoreCase(subStatusVO.getId(),
						YorosisContext.get().getTenantId(), YorosisConstants.YES);
				Timestamp timestamp = new Timestamp(System.currentTimeMillis());
		...
		List<TaskboardSubStatus> taskboardSubStatusList = column.getTaskboardSubStatus().stream()
				.filter(task -> StringUtils.equals(task.getActiveFlag(), YorosisConstants.YES)).collect(Collectors.toList());
		if ((taskboardSubStatusList == null || taskboardSubStatusList.isEmpty()) && taskboardSubStatusVO.getDeletedIdList().isEmpty()) {
	...
	private void removeSubStatus(List<UUID> deletedSubStatusIdList) {
		List<TaskboardSubStatus> subStatus = taskboardSubStatusRepository.getIdAndTenantIdIgnoreCaseAndActiveFlagIgnoreCase(deletedSubStatusIdList,
				YorosisContext.get().getTenantId(), YorosisConstants.YES);
	...
	public List<SubStatusVO> getSubStatusList(UUID taskboardColumnId) {
		TaskboardColumns column = taskboardColumnsRepository.findByIdAndTenantIdIgnoreCaseAndActiveFlagIgnoreCase(taskboardColumnId,
				YorosisContext.get().getTenantId(), YorosisConstants.YES);
		return taskboardSubStatusRepository.getSubStatusList(column, YorosisContext.get().getTenantId(), YorosisConstants.YES).stream()
	...
	private void setSubStatus(String subStatusName, TaskboardColumns column) {
		List<TaskboardTask> taskboardTaskList = taskboardTaskRepository.findByTaskboardAndStatusAndTenantIdIgnoreCaseAndActiveFlagIgnoreCase(
				column.getTaskboard(), column.getColumnName(), YorosisContext.get().getTenantId(), YorosisConstants.YES);
	...
	private void updateSubStatusNameInTask(List<SubStatusVO> previousSubStatusList, TaskboardColumns column) {
		for (SubStatusVO subStatus : previousSubStatusList) {
			List<TaskboardTask> taskboardTaskList = taskboardTaskRepository.getTaskboardTaskByPreviousSubStatusName(column.getTaskboard(),
					column.getColumnName(), subStatus.getPreviousName(), YorosisContext.get().getTenantId(), YorosisConstants.YES);
